...
namespace RecursosHumanosCore.Controller
{
    public class PuestoController
    {
        ...
        public (int id, string mensaje) RegistrarPuesto(Puesto puesto, int idUsuario)
        {
            try
            {
                ...
                auditoriasController.RegistrarAuditoriaGenerica(2, 1, (short)idPuesto, idUsuario); // Ahora se pasa el id del usuario
        ...
        public (bool exito, string mensaje) ExportarPuestosExcel(bool estatus)
        {
            try
            {
                var puestos = _puestoDataAccess.ObtenerTodosLosPuestos();
                ...
                if (resultado)
                {
                    ...
                    return (true, "La exportación de puestos a Excel se completó exitosamente.");
                }
                else
                {
                    _logger.Warn("No se pudo exportar el archivo.");
                    return (false, "La exportación de puestos a Excel ha fallado.");
                }
            }
            catch (Exception ex)
            {
                _logger.Error(ex, "Error al exportar puestos a Excel");
                return (false, "Ocurrió un error inesperado al exportar los puestos.");
            }
        }
