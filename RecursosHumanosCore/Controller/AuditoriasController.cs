...
namespace RecursosHumanosCore.Controller
{
    public class AuditoriasController
    {
        ...
        public (bool exito, string mensaje) RegistrarAuditoriaGenerica(int idEntidad, int idAccion, short movimiento, int idUsuario)
        {
            try
            {
                // Obtener el nombre y la IP del equipo
        ...
        public (bool exito, string mensaje, string rutaArchivo) ExportarAuditoriasExcel(int? idTipo = null, int? idAccion = null,
                                 DateTime? fechaInicio = null, DateTime? fechaFin = null,
                                 string ipEquipo = null, string nombreEquipo = null,
                                 int? idUsuario = null, int? estatus = null)
        {
            try
            {
                ...
                if (auditorias == null || auditorias.Count == 0)
                {
                    ...
                    return (false, "No se encontraron auditorías para exportar.", null);
                }
                ...
                if (resultado)
                {
                    ...
                    return (true, "La exportación a Excel se completó exitosamente.", rutaArchivo);
                }
                else
                {
                    _logger.Warn("No se pudo exportar el archivo.");
                    return (false, "No se pudo exportar el archivo.", null);
                }
            }
            catch (Exception ex)
            {
                _logger.Error(ex, "Error al exportar auditorías a Excel");
                return (false, "Ocurrió un error inesperado durante la exportación.", null);
            }
        }
