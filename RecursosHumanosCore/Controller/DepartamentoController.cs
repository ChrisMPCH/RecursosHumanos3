...
namespace RecursosHumanosCore.Controller
{
    public class DepartamentoController
    {
        ...
        public (int id, string mensaje) RegistrarDepartamento(Departamento departamento, int idUsuario)

        {
            try
            {
                ...
                _auditoriasController.RegistrarAuditoriaGenerica(2, 1, (short)idDepartamento, idUsuario);
        ...
        public (bool exito, string mensaje, string rutaArchivo) ExportarDepartamentosExcel(bool estatus)
        {
            try
            {
                var departamentos = _departamentoDataAccess.ObtenerTodosLosDepartamentos();

                Func<Departamento, bool> filtro = d => d.Estatus == estatus;
                var nombre = $"Departamentos_{DateTime.Now:yyyyMMdd_HHmmss}.xlsx";

                string rutaArchivo = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Desktop), "exportados", nombre);
                ...
                if (resultado)
                {
                    ...
                    return (true, "La exportación de departamentos a Excel se completó exitosamente.", rutaArchivo);
                }
                else
                {
                    _logger.Warn("No se pudo exportar el archivo.");
                    return (false, "La exportación de departamentos a Excel ha fallado.", "");
                }
            }
            catch (Exception ex)
            {
